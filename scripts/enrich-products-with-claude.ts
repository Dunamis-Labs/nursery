#!/usr/bin/env tsx
/**
 * Product Enrichment Script using Claude API
 * 
 * This script iterates through all products and uses Claude API to:
...
/**
 * Call Claude API to enrich product data
 */
async function enrichProductWithClaude(product: {
  id: string;
  name: string;
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          model: 'claude-3-opus-20240229', // Using Claude 3 Opus (Sonnet not available with this API key)
          max_tokens: 4096,
          messages: [
            {
              role: 'user',
      ...
      const result = JSON.parse(jsonText.trim()) as ClaudeResponse;
 ...
 */
async function main() {
  console.log('🌱 Starting Product Enrichment with Claude API...\n');
  ...
  const SKIP_ENRICHED = process.env.SKIP_ENRICHED !== 'false'; // Default to true - skip already enriched products
  const limit = TEST_MODE ? 1 : undefined;
  
  // Build where clause to optionally skip already-enriched products
  const whereClause: any = {};
  
  if (SKIP_ENRICHED && !TEST_MODE) {
    // Skip products that already have complete enrichment
    whereClause.OR = [
      { content: null }, // No ProductContent
      { specification: null }, // No ProductSpecification
      { faqs: { none: {} } }, // No FAQs
      { content: { detailedDescription: null } }, // Missing description
      { content: { careInstructions: null } }, // Missing care instructions
    ];
    console.log('⏭️  SKIP_ENRICHED mode: Will skip products that are already fully enriched\n');
  }
  ...
  const productsToProcess = products.length;
  if (SKIP_ENRICHED && !TEST_MODE && productsToProcess < totalProducts) {
    const alreadyEnriched = totalProducts - productsToProcess;
    console.log(`📊 Found ${productsToProcess} products to enrich (${alreadyEnriched} already enriched, skipping)\n`);
  }
  ...
  for (const product of products) {
    processed++;
    const progress = productsToProcess > 0 ? ((processed / productsToProcess) * 100).toFixed(1) : '0.0';

    console.log(`\n[${processed}/${productsToProcess}] (${progress}%) Processing: ${product.name}`);
    console.log(`  ID: ${product.id}`);

    try {
      // Enrich with Claude
      const enrichedData = await enrichProductWithClaude(product);
  ...
  console.log(`Products Processed: ${productsToProcess}`);
  if (SKIP_ENRICHED && !TEST_MODE) {
    const alreadyEnriched = totalProducts - productsToProcess;
  ...
  if (productsToProcess > 0) {
    console.log(`📈 Success Rate: ${((succeeded / productsToProcess) * 100).toFixed(1)}%`);
  }
